...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const soldTickets = await ctx.db
      ...
      .filter((q) =>
        q.or(q.eq(q.field("status"), "vails"), q.eq(q.field("status"), "used")),
      )
      .collect();
...
//  Purchase ticket
console.log("🔥 purchaseTicket mutation loaded");
export const purchaseTicket = mutation({
  ...
  handler: async (ctx, { eventId, userId, waitingListId, paymentInfo }) => {
    ...
    if (!event) {
      console.error("Event notfound", { eventId });
...
//  Get useris tickets with events information
